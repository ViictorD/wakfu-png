...
use super::{anm_shape::AnmShapeTrait, anm_shape_ta::AnmShapeTA};
use anyhow::Result;

#[derive(Clone)]
pub struct AnmShapeTAM {
	pub shape_ta: AnmShapeTA,
	pub mul_red: i8,
	pub mul_green: i8,
	pub mul_blue: i8,
	pub mul_alpha: i8
...
impl AnmShapeTrait for AnmShapeTAM {
	...
	fn process(&self, parent: &AnmTransform, result: &mut AnmTransform) {
		...
		let tx = self.shape_ta.shape_t.translation_x as f32 / 16.;
		let ty = self.shape_ta.shape_t.translation_y as f32 / 16.;
		...
		result.red = parent.red * (self.mul_red as f32 * 0.007874016) + self.shape_ta.add_red as f32 * 0.00390625;
		result.green = parent.green * (self.mul_green as f32 * 0.007874016) + self.shape_ta.add_green as f32 * 0.00390625;
		result.blue = parent.blue * (self.mul_blue as f32 * 0.007874016) + self.shape_ta.add_blue as f32 * 0.00390625;
		result.alpha = parent.alpha * (self.mul_alpha as f32 * 0.007874016) + self.shape_ta.add_alpha as f32 * 0.00390625;
	}
}