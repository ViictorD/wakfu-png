...
use super::{anm_shape::AnmShapeTrait, anm_shape_rt::AnmShapeRT};
use anyhow::Result;

#[derive(Clone)]
pub struct AnmShapeRTM {
	pub shape_rt: AnmShapeRT,
	pub mul_red: i8,
	pub mul_green: i8,
	pub mul_blue: i8,
	pub mul_alpha: i8
...
impl AnmShapeTrait for AnmShapeRTM {
	...
	fn process(&self, parent: &AnmTransform, result: &mut AnmTransform) {
		...
		let tx = self.shape_rt.translation_x as f32 / 16.;
		let ty = self.shape_rt.translation_y as f32 / 16.;
		let rx0 = self.shape_rt.shape_r.rotation_skew_x0 as f32 / 256.;
		let ry0 = self.shape_rt.shape_r.rotation_skew_y0 as f32 / 256.;
		let rx = self.shape_rt.shape_r.rotation_skew_x1 as f32 / 256.;
		let ry = self.shape_rt.shape_r.rotation_skew_y1 as f32 / 256.;
